...
public class InsertHang extends myframe.MyFrame {

    private MyConnection myConnection;
    private CommonValue myCommonValue;
    
    private ArrayList<String>arr=new ArrayList<String>();
    public InsertHang(MyConnection myConnection,ResultSet resultSet) {
        initComponents();
        int w=getWidth();
        int h=getHeight();
        setBackground();
        ...
        try {
                while (resultSet.next()) {                
                    cbTenNcc.addItem(resultSet.getString("NAME"));
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Thêm Mặt Hàng");
    ...
    private void btOk1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btOk1ActionPerformed
        ...
        String ten=cbTenNcc.getSelectedItem().toString().trim();
        String idNcc=arr.get(cbTenNcc.getSelectedIndex());
        if (!name.equals("")
            && !gia.equals("")
            && !lai.equals("")) {
            try {
                myConnection.updateSql(myCommonValue.InsertHang(name, gia, lai, idNcc));
